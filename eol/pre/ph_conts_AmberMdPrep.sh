...
### Modificaciones p/ pH constante. P/ q se usen las cargas correctas,
## pero no intente intercambiar estados
# icnstph = 2, ntcnstph = 100000,

## use of cpin is hardcoded. Check around lines:  ~731(minimization) and ~820(MD)
...
Minimization: $MDIN
 &cntrl
   ...
   ntwx = $NTWX, ioutfm = 1, ntxo = 1, ntpr = $NTPR, ntwr = $NTWR, 
   ntc = 1, ntf = 1, ntb = 1, cut = $CUT, icnstph = 2, ntcnstph = 100000,
EOF
  CharmmWater
  RestraintLine
cat >> $MDIN <<EOF
...
EOF
  ...
  if [ $OVERWRITE -eq 1 -o ! -f "$RST" ] ; then
    ...
    if [ $TEST -eq 0 ] ; then
      $PROG_MIN -O -i $MDIN -p $TOP -c $CRD -ref $REF -o $RUN.out -x $RUN.nc -r $RST -inf $RUN.mdinfo -cpin $NAMECPIN
    else
...
MD: $MDIN
 &cntrl
   ...
   ntwx = $NTWX, ntwv = -1, ioutfm = 1, ntxo = 1, ntpr = $NTPR, ntwr = $NTWR, 
   iwrap = 0, nscm = $NSCM,
   ntc = 2, ntf = 2, ntb = $NTB, cut = $CUT, icnstph = 2, ntcnstph = 100000, 
EOF
  Thermostat $MDIN
  if [ $NTB -eq 2 ] ; then
    Barostat $MDIN
...
EOF
  ...
  if [ $OVERWRITE -eq 1 -o ! -f "$RST" ] ; then
    echo "MD: $RUN"
    if [ $TEST -eq 0 ] ; then
      $PROG_MD -O -i $MDIN -p $TOP -c $CRD -ref $REF -o $RUN.out -x $RUN.nc -r $RST -inf $RUN.mdinfo -cpin $NAMECPIN
    else
