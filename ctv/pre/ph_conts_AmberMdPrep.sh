...
## cpin is hardcoded. Check around lines:  ~731(minimization) and ~820(MD)
...
EOF
  ...
  if [ $OVERWRITE -eq 1 -o ! -f "$RST" ] ; then
    ...
    if [ $TEST -eq 0 ] ; then
      $PROG_MIN -O -i $MDIN -p $TOP -c $CRD -ref $REF -o $RUN.out -x $RUN.nc -r $RST -inf $RUN.mdinfo -cpin ../top/cpin_ctv
    else
...
EOF
  ...
  if [ $OVERWRITE -eq 1 -o ! -f "$RST" ] ; then
    echo "MD: $RUN"
    if [ $TEST -eq 0 ] ; then
      $PROG_MD -O -i $MDIN -p $TOP -c $CRD -ref $REF -o $RUN.out -x $RUN.nc -r $RST -inf $RUN.mdinfo -cpin ../top/cpin_ctv
    else
